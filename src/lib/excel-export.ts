...
import { analyzeStudent, classCategoryAverages } from "./analysis";
import { formatJalaliWithWeekday } from "./jalali";
import { studentWeeklyComputed, previousLevelFor, selectWeekDailies } from "./store";

const INK = "FF1E4D3A";
const CREAM = "FFF4F1EA";
const PAPER = "FFF3EFE6";
const WHITE = "FFFFFCF7";
const INPUT = "FFFFF8E1";
const LINE = "FFE4DDD2";
const MUTED = "FF6B645C";
...
function font(opts: Partial<ExcelJS.Font> = {}) {
  return { name: "Tahoma", size: 10, ...opts };
}
...
function headerCell(cell: ExcelJS.Cell, text: string) {
  cell.value = text;
  cell.font = font({ bold: true, color: { argb: CREAM }, size: 10 });
  cell.fill = fill(INK);
  cell.alignment = { horizontal: "center", vertical: "middle", wrapText: true };
  cell.border = thin;
}

function inputCell(cell: ExcelJS.Cell, value: ExcelJS.CellValue) {
  cell.value = value;
  cell.font = font();
...
function valueCell(cell: ExcelJS.Cell, value: ExcelJS.CellValue, bold = false) {
  cell.value = value;
  cell.font = font({ bold });
  cell.alignment = { horizontal: "center", vertical: "middle" };
  cell.border = thin;
...
function titleRow(ws: ExcelJS.Worksheet, text: string, cols: number) {
  ws.mergeCells(1, 1, 1, cols);
  const c = ws.getCell(1, 1);
  c.value = text;
  c.font = font({ bold: true, size: 16, color: { argb: INK } });
...
export async function buildWorkbook(state: AppState): Promise<ExcelJS.Workbook> {
...
export async function downloadExcel(state: AppState) {
  const wb = await buildWorkbook(state);
...
function rtlSheet(wb: ExcelJS.Workbook, name: string, landscape = true) {
...
function addGuide(wb: ExcelJS.Workbook, state: AppState) {
  ...
  ws.getCell("A2").font = font({ bold: true, color: { argb: INK } });
  ...
  lines.forEach(([k, v], i) => {
    ...
    ws.getCell(r, 1).font = font({ bold: true, color: { argb: INK } });
    ws.getCell(r, 2).value = v;
    ws.getCell(r, 2).font = font();
    ws.getCell(r, 2).alignment = { wrapText: true, vertical: "middle", horizontal: "right" };
    ws.getRow(r).height = 28;
  });
}

function addDashboard(wb: ExcelJS.Workbook, state: AppState) {
  const ws = rtlSheet(wb, "داشبورد");
  const week = state.weeks.find((w) => w.id === state.activeWeekId);
  titleRow(ws, `داشبورد کلاس — ${week?.label ?? ""}`, 9);
  ...
  const headers = ["رتبه", "نام", "میانگین هفته", "شفاهی", "کتبی", "انضباط", "نمره کل", "سطح", "توضیح"];
  headers.forEach((h, i) => headerCell(ws.getCell(4, i + 1), h));
  ...
  rows.forEach((r, idx) => {
    ...
    nameCell.font = font({ underline: true, color: { argb: INK } });
  ...
  [8, 28, 14, 12, 12, 12, 12, 12, 22].forEach((w, i) => {
...
function addRoster(wb: ExcelJS.Workbook, state: AppState) {
  const ws = rtlSheet(wb, "دانش‌آموزان");
  titleRow(ws, "فهرست پرونده‌ها — خانه‌های زرد قابل ویرایش‌اند", 9);
  const headers = ["ردیف", "نام", "نام پدر", "تلفن", "سال تولد", "شهر", "جزء جاری", "سطح", "امتیاز"];
  headers.forEach((h, i) => headerCell(ws.getCell(3, i + 1), h));
  state.students.forEach((s, i) => {
    const r = 4 + i;
    valueCell(ws.getCell(r, 1), i + 1);
    const name = ws.getCell(r, 2);
    name.value = { text: s.fullName, hyperlink: `#'${sheetNameFor(s.fullName)}'!A1` };
    name.font = font({ underline: true, color: { argb: INK } });
  ...
  [8, 28, 16, 16, 12, 14, 12, 12, 12].forEach((w, i) => {
    ws.getColumn(i + 1).width = w;
  });
}

function addDailyForm(wb: ExcelJS.Workbook, state: AppState) {
  const week = state.weeks.find((w) => w.id === state.activeWeekId);
  if (!week) return;
  ...
  week.sessionDates.forEach((date) => {
    ...
    t.font = font({ bold: true, size: 12, color: { argb: INK } });
    t.fill = fill(PAPER);
    row += 1;

    const headers = ["ردیف", "نام", "حضور", ...SCORE_FIELDS.map((f) => `${f.short}\n${f.max}`), "جمع ۱۴۰", "انضباط ۶۰", "میانگین ۱۰۰"];
...
function addWeeklyForm(wb: ExcelJS.Workbook, state: AppState) {
  const week = state.weeks.find((w) => w.id === state.activeWeekId);
  if (!week) return;
  ...
  state.students.forEach((s, i) => {
    ...
    exam.value = { formula: `IF(AND(D${r}="",E${r}=""),"",IF(D${r}="",0,D${r})+IF(E${r}="",0,E${r}))`, result: c.examTotal ?? 0 };
  ...
  [8, 26, 14, 12, 12, 12, 12, 14, 28, 12].forEach((w, i) => {
    ws.getColumn(i + 1).width = w;
  });
}

function addDailyData(wb: ExcelJS.Workbook, state: AppState) {
  ...
  headers.forEach((h, i) => headerCell(ws.getCell(3, i + 1), h));
  const sorted = [...state.dailies].sort((a, b) => {
...
function addAnalysis(wb: ExcelJS.Workbook, state: AppState) {
  const ws = rtlSheet(wb, "تحلیل عملکرد", false);
  titleRow(ws, "تحلیل کلاس و برنامه فردی هفته بعد", 4);
  const days = selectWeekDailies(state, state.activeWeekId).filter(
  ...
  ws.getCell(r, 1).font = font({ bold: true, size: 13, color: { argb: INK } });
  r += 1;
  ...
  for (const s of state.students) {
    const insight = analyzeStudent(
      s,
      state.dailies.filter((d) => d.studentId === s.id),
      state.weeklies.filter((w) => w.studentId === s.id),
    );
    valueCell(ws.getCell(r, 1), s.fullName);
    ws.getCell(r, 1).alignment = { horizontal: "right", wrapText: true };
    valueCell(ws.getCell(r, 2), insight.focusTitle);
    ws.getCell(r, 2).alignment = { wrapText: true, horizontal: "right" };
    valueCell(ws.getCell(r, 3), insight.paragraphs.join(" "));
    ws.getCell(r, 3).alignment = { wrapText: true, horizontal: "right" };
    valueCell(ws.getCell(r, 4), insight.plan.join(" · "));
    ws.getCell(r, 4).alignment = { wrapText: true, horizontal: "right" };
...
function addStudentSheet(wb: ExcelJS.Workbook, state: AppState, s: Student) {
  ...
  const info: Array<[string, ExcelJS.CellValue]> = [
  ...
  ws.getCell(16, 1).font = font({ bold: true, size: 12, color: { argb: INK } });
  ws.mergeCells(17, 1, 19, 6);
  ws.getCell(17, 1).value = [...insight.paragraphs, ...insight.plan.map((p, i) => `${i + 1}. ${p}`)].join("\n");
  ws.getCell(17, 1).alignment = { wrapText: true, vertical: "top", horizontal: "right" };
  ws.getCell(17, 1).font = font();
  ws.getRow(17).height = 36;
  ws.getRow(18).height = 36;
  ws.getRow(19).height = 36;

  const headers = ["هفته", "میانگین", "شفاهی", "کتبی", "نمره کل", "وضعیت"];
  headers.forEach((h, i) => headerCell(ws.getCell(21, i + 1), h));
  ...
  ws.getCell(start, 1).font = font({ bold: true, size: 12, color: { argb: INK } });
  const dh = ["تاریخ", ...SCORE_FIELDS.map((f) => f.short), "جمع", "انضباط", "میانگین"];
  dh.forEach((h, i) => headerCell(ws.getCell(start + 1, i + 1), h));
  const days = state.dailies
