  ...
  persist(
    (set, get) => ({
      ...
      lockWeek: (weekId) => {
        ...
        type Row = {
          studentId: string
          prevLevel: import("./types").Level
          computed: ReturnType<typeof computeWeekly>
          isEthics: boolean
        };
        const rows: Row[] = [];
        ...
        for (const s of state.students) {
          ...
          rows.push({ studentId: s.id, prevLevel, computed, isEthics: false });
        }

        const maxDisc = Math.max(0, ...discByStudent.values());
        for (const r of rows) {
          r.isEthics =
            r.computed.disciplineAvg != null &&
            r.computed.disciplineAvg === maxDisc &&
            maxDisc > 0;
        }

        const transfers = applyLevelTransfers(
          rows.map((r) => ({
            studentId: r.studentId,
            prevLevel: r.prevLevel,
            weekTotal: r.computed.weekTotal,
            cardType: r.computed.card?.type ?? null,
          })),
        );
        ...
        for (const r of rows) {
          const transfer = transfers.get(r.studentId) ?? {
            nextLevel: r.prevLevel,
            change: "stay" as const,
          };
          const computed = {
            ...r.computed,
            nextLevel: transfer.nextLevel,
            change: transfer.change,
          };

          let awarded = performancePoints(computed.weekTotal, settings);
          if (r.isEthics) awarded += settings.ethicsBonus;
          if (computed.change === "promote") awarded += settings.promotionBonus;
          if (computed.card) awarded += computed.card.points;

          const penalty = absencePenalty(computed.nonExcused, computed.excused);
          awarded -= penalty;

          weeklies = weeklies.map((w) =>
            w.studentId === r.studentId && w.weekId === weekId
              ? {
                  ...w,
                  locked: true,
                  isEthicsMan: r.isEthics,
          ...
          if (r.isEthics) reasonParts.push("مرد اخلاق");
          if (computed.change === "promote") reasonParts.push("ارتقاء سطح");
          if (computed.change === "demote") reasonParts.push("تنزل سطح");
          if (computed.card) reasonParts.push(`کارت ${computed.card.label}`);
          if (penalty > 0) reasonParts.push(`جریمه غیبت (-${penalty})`);

          if (awarded !== 0 || penalty > 0 || computed.card || computed.change !== "stay") {
            points.push({
              id: `p-${r.studentId}-${weekId}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
              studentId: r.studentId,
              weekId,
              date: week.evalDate,
              amount: awarded,
...
export function studentWeeklyComputed(state: AppState, studentId: string, weekId: string) {
  ...
  const base = computeWeekly(days, weekly, prev, state.settings);
  if (weekly?.locked && weekly.levelAfter) {
    return {
      ...base,
      nextLevel: weekly.levelAfter,
      change: weekly.levelChange ?? "stay",
      cleaningDuty: weekly.levelAfter === "C" || (weekly.levelAfter == null && prev === "C"),
    };
  }
  return base;
}
